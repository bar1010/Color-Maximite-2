...
'***************************
width = 11 : height = 11   '101x51, 201x99, 301x149, width and height must be ODD
'***************************

floor = 1 : have_key = 0 : score = 0 : max_score = 0
generate()

sub generate
  seed = int(rnd * 1234567890)
  'dim mg(width, height) 'index out of bounds
  for y = 1 to height  
    for x = 1 to width   
      mg(x, y) = 0
    next x   
  ...
  '  for xt = 0 to width - 3
  '    if m$(xt, yt) <> wc$ and m$(xt, yt) <> "x" and m$(xt, yt) <> "d" and m$(xt, yt) <> "k" then
  '      m$(xt, yt) = "." 
  '      maxscore = maxscore + 1
  '    endif
  '  next xt
...
'sub detect
  do
    ...
    elseif keydown(1) = 100 then 'user presses D key to go down a floor
      'print m$(xa, ya);"  have key=";have_key;
...
sub draw_2d_maze
  if screen_drawn = 0 and drawn_maze = 0 then 'just need one variable here
    ...
    if have_key = 0 then
      print @(9 * 8, 49 * 12) "Floor =";floor;"  no key";
    elseif have_key = 1 then
      print @(9 * 8, 49 * 12) "Floor =";floor;"  key";
    endif
    color Indigo
    screen_drawn = 1
    drawn_maze = 1
    ...
    if have_key = 0 then
      print @(16 * 8, 49 * 12) floor;"  no key";
    elseif have_key = 1 then
      print @(16 * 8, 49 * 12) floor;"  key   ";
    endif
    color Indigo
    pause 64
  endif
...
sub draw_3d_maze
  ...
  if have_key = 0 then
    print @(399, 12) ch$;space$(10);floor;"  no key"
  elseif have_key = 1 then
    print @(399, 12) ch$;space$(10);floor;"  key"
  endif
  pause 200
end sub
