...
#include "colors.inc"
option base 0
dim integer width, height, depth, view, done
...
dim integer score, maxscore, d, f, seed, sx, sy, sz
...
dim string m$(48, 48, 48) length 1 'Maze array
...
for zt = 0 to depth - 3
  for yt = 0 to height - 3
    for xt = 0 to width - 3
      if m$(xt, yt, zt) <> wc$ and m$(xt, yt, zt) <> "x" and m$(xt, yt, zt) <> uc$ and m$(xt, yt, zt) <> dc$ and m$(xt, yt, zt) <> bc$ then
        m$(xt, yt, zt) = "." 
        maxscore = maxscore + 1
      endif
    next xt
...
do
  ...
  elseif keydown(1) = 128 then 'User presses the cursor forward key
    if view = 3 then
      select case ch$
        case "W"
          if m$(xa - 1, ya, za) <> wc$ then 
          if m$(xa - 1, ya, za) = "." then score = score + 1
        ...
        case "N"
          if m$(xa, ya - 1, za) <> wc$ then 
            if m$(xa, ya - 1, za) = "." then score = score + 1
  ...
  elseif keydown(1) = 136 then 'user presses Page Up key to climb UP one floor
    if view = 3 then
      if m$(xa, ya, za - 1) <> wc$ then 
        if m$(xa, ya, za - 1) = "." then score = score + 1
  ...
  elseif keydown(1) = 137 then 'user presses Page Down key to climb DOWN one floor
    if xa = width - 4 and ya = height - 4 and za = depth - 4 then
      cls : print "You found the exit!" : pause 5000 : end
    endif
    if view = 3 then
      if m$(xa, ya, za + 1) <> wc$ then 
        if m$(xa, ya, za + 1) = "." then score = score + 1
...
sub draw_2d_maze
  if screen_drawn = 0 and drawn_maze = 0 then 'just need one variable here
    ...
    for yt = ytlow to 31 'ythigh
      for xt = xtlow to 31 'xthigh
    ...
    print @(32 * 8, 49 * 12) za;
    if m$(xa, ya, za) = "e" then cls : print "You found the exit!" : ? : ?
    pause 64
  endif
end sub
...
sub draw_3d_maze
  ...
  zt = za
  if m$(xt, yt, zt) = "." then circle 399, 525, 32
  ...
  if draw$ = "Y" then
    ...
    if m$(xt, yt, zt) = "." then circle 399, 413, 16
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if m$(xt, yt, zt) = "." then circle 399, 356, 8
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if zt < 0 then zt = 0
    if m$(xt, yt, zt) = "." then circle 399, 328, 4
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if m$(xt, yt, zt) = "." then circle 399, 314, 2
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if m$(xt, yt, zt) = "." then circle 399, 308, 1
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if m$(xt, yt, zt) = "." then circle 399, 304, 1
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if m$(xt, yt, zt) = "." then circle 399, 302, 1
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
  ...
  if draw$ = "Y" then
    ...
    zt = za
    if m$(xt, yt, zt) = "." then circle 399, 301, 1
    if m$(xt, yt, zt) = uc$ or m$(xt, yt, zt) = dc$ or m$(xt, yt, zt) = bc$ then 
