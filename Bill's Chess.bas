...
do
  ...
  for i = 1 to 8
    for j = 1 to 8
      bhistory(8, 8, half_moves) = board(i, j)
    next j
  next i
  if whos_move$ = "W" then
    whos_move$ = "B"
    ...
    for d = 1 to 8
      if board(c, d) = 1 then 
        ...
        if half_moves > 2 and m1 - 1 > 0 then
          if m2 = 5 and board(m1 - 1, 5) = -1 and m3 = m1 - 1 and m4 = 6 then
            control_board(c, d, c - 1, d + 1, 1)
            if bcb(wkf, wkr) = 0 and bhistory(m3, 7, half_moves - 1) = -1 then 'en passant to the left
        ...
        if half_moves > 2 and m1 + 1 < 9 then
          if m2 = 5 and board(m1 + 1, 5) = -1 and m3 = m1 + 1 and m4 = 6 then
            control_board(c, d, c + 1, d + 1, 1)
            if bcb(wkf, wkr) = 0 and bhistory(m3, 7, half_moves - 1) = -1 then 'en passant to the right
              pml$(m) = ntof(c) + "5" + ntof(c + 1) + "6"
            endif
          endif
        endif
  ...
  for c = 1 to 8
    for d = 1 to 8
      if board(c, d) = -1 then 
        ...
        if half_moves > 2 and m1 - 1 > 0 then
          if m2 = 4 and board(m1 - 1, 4) = 1 and m3 = m1 - 1 and m4 = 3 then 'changed from = -1 to = 1.  Does this fix the bug?
            control_board(c, d, c - 1, d - 1, -1)
            if wcb(bkf, bkr) = 0 and bhistory(m3, 2, half_moves - 1) = 1 then 'en passant to the left
        ...
        if half_moves > 2 and m1 + 1 < 9 then
          if m2 = 4 and board(m1 + 1, 4) = 1 and m3 = m1 + 1 and m4 = 3 then 'changed from= -1 to = 1.  Does this fix the bug?
            control_board(c, d, c + 1, d - 1, -1)
            if wcb(bkf, bkr) = 0 and bhistory(m3, 2, half_moves - 1) = 1 then 'en passant to the right
              pml$(m) = ntof(c) + "4" + ntof(c + 1) + "3"
            endif
          endif
        endif
...
sub determine_result
  ...
  'I made a change here.  Does it work correctly now?
